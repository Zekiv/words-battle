// script_2d.js (Complete - Refined Waiting State Logic)
...
let isCurrentlyWaiting = false; // Flag for waiting state
let gameMessageTimeout = null;
let currentWaitingMessage = "Waiting for Opponent..."; // Store the message
...
function drawWaitingText() { // Takes message from global variable now
    if (!ctx || !canvas) return;
    // Background is drawn first by gameLoop
    const message = currentWaitingMessage; // Use the stored message
    const fontSize = 28; ctx.font = `bold ${fontSize}px 'Segoe UI', sans-serif`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    const textWidth = ctx.measureText(message).width;
    const boxPadding = 20; const boxWidth = textWidth + boxPadding * 2; const boxHeight = fontSize + boxPadding * 1.5;
    const boxX = (canvas.width - boxWidth) / 2; const boxY = (canvas.height - boxHeight) / 2 - 20;
    ctx.fillStyle = "rgba(0, 0, 0, 0.7)"; ctx.shadowColor = "rgba(0, 0, 0, 0.5)"; ctx.shadowBlur = 5; ctx.shadowOffsetX = 3; ctx.shadowOffsetY = 3;
    // Ensure rect doesn't go off-canvas if text is very long
    ctx.fillRect(Math.max(0, boxX), boxY, Math.min(canvas.width, boxWidth), boxHeight);
    ctx.shadowColor = "transparent"; ctx.shadowBlur = 0; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0;
    ctx.fillStyle = "#FFFFFF"; ctx.fillText(message, canvas.width / 2, boxY + boxHeight / 2);
}

function resetUI(statusMsg = 'Initializing...') {
    try {
        isCurrentlyWaiting = false; // <<< Reset waiting flag
        ...
        // Reset join button state based on WebSocket status
        const joinDisabled = !(ws && ws.readyState === WebSocket.OPEN);
...
// --- Game Logic & State Updates --- (Keep All functions)
function submitWord() { /* ... */ } function updateLettersUI(l) { /* ... */ } function updateHpUI(id,hp,max=100) { /* ... */ }
...
function displayWordFeedback(m, type='info') { /* ... */ } function startRoundTimer(ms) { /* ... */ } function spawnSoldiers(oid, ct) { /* ... */ } function removeSoldiersByIds(ids) { /* ... */ } function showGameOver(m) { try { console.log("CLIENT: showGameOver:", m); isCurrentlyWaiting=false; gameActive=false; stopGameLoop(); clearInterval(currentTimerInterval); if(gameOverMessage) gameOverMessage.textContent=m; if(gameOverScreen) gameOverScreen.style.display='block'; if(player1InfoBox) player1InfoBox.style.display='none'; if(player2InfoBox) player2InfoBox.style.display='none'; if(centerInfoBox) centerInfoBox.style.display='none'; if(playerControlsBox) playerControlsBox.style.display='none'; if(wordHistoryBox) wordHistoryBox.style.display='none'; if(nicknamePrompt) nicknamePrompt.style.display='none'; activeSoldiers=[]; if(ctx){clearCanvas(); drawBackground();} } catch(e){console.error("CLIENT: Err showGameOver:",e);} }
function updateWordHistoryUI(playedWords) { /* ... Keep as is ... */ }

// --- Canvas Drawing Functions --- (Keep drawCastle, drawSoldier)
function drawCastle(x,y,w,h,c) { /* ... */ } function drawSoldier(s) { /* ... */ }

// --- Animation Loop & Game Updates (Refined Logic) ---
function gameLoop() {
    try {
        clearCanvas();
        drawBackground();
        ...
        // Continue the loop ONLY if waiting or game is active
        if (isCurrentlyWaiting || gameActive) {
            animationFrameId = requestAnimationFrame(gameLoop);
        } else {
             stopGameLoop(); // Explicitly stop if neither state applies
        }
    } catch (error) {
        console.error("CLIENT: Error in gameLoop:", error);
        gameActive = false; isCurrentlyWaiting = false; // Reset flags
        showGameOver("Runtime Error!"); // Show game over on loop error
        stopGameLoop(); // Ensure loop stops
    }
}

function startGameLoop() {
    // Starts the loop regardless of gameActive, loop itself decides what to draw
    try {
        if (!animationFrameId) {
            console.log("CLIENT: Starting animation loop.");
            // isCurrentlyWaiting or gameActive will be set by the message handler BEFORE this is called
            animationFrameId = requestAnimationFrame(gameLoop);
        } else {
...
function moveSoldiers() { /* ... Keep as is ... */ } function checkCombat() { /* ... Keep as is ... */ } function checkCastleHits() { /* ... Keep as is ... */ }
     ...
     try {
        joinButton?.addEventListener('click', () => {
            ...
            if (nick && ws && ws.readyState === WebSocket.OPEN) {
                ...
                // <<< Do NOT show waiting overlay div >>>
                // <<< Just disable buttons, wait for 'waiting' message >>>
...
function connectWebSocket() {
    try {
        ...
        ws.onclose = (e) => { console.log(`CLIENT: WS Closed. Code:${e.code}`); isCurrentlyWaiting = false; gameActive=false; stopGameLoop(); clearInterval(currentTimerInterval); resetUI('Disconnected. Refresh?'); ws=null; playerId=null; gameId=null; playerNumber=null; };
...
function handleServerMessage(message) {
    try {
        switch (message.type) {
            ...
            case 'opponentLeft': if (!gameActive && gameOverScreen?.style.display !== 'none') break; displayGameMessage(`${opponentNickname} has left.`, 'error'); break; // Server usually sends gameOver after
...
function init() { // Define init BEFORE DOMContentLoaded calls it
    try {
        console.log("CLIENT: init() started (Draw Waiting Text V3).");
        // Assign DOM elements
        ...
        setupUIListeners(); // Setup listeners AFTER elements assigned
        resetUI('Initializing...'); // Reset UI state AFTER elements assigned
