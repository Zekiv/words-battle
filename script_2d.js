// script_2d.js (FINAL - Auto-clear Game Messages)
...
let gameMessageTimeout = null; // <<< Variable to hold the timeout ID
...
function drawWaitingText(message = "Waiting for Opponent...") { if (!ctx || !canvas) return; drawBackground(); ctx.fillStyle="rgba(0,0,0,0.7)"; ctx.fillRect(0, canvas.height*0.4, canvas.width, 60); ctx.font="bold 28px 'Segoe UI', sans-serif"; ctx.fillStyle="#FFFFFF"; ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.shadowColor="rgba(0,0,0,0.5)"; ctx.shadowBlur=4; ctx.shadowOffsetX=2; ctx.shadowOffsetY=2; ctx.fillText(message, canvas.width/2, canvas.height/2); ctx.shadowColor="transparent"; ctx.shadowBlur=0; ctx.shadowOffsetX=0; ctx.shadowOffsetY=0; }
...
// --- Updated displayGameMessage Function ---
function displayGameMessage(message, type = 'info', durationMs = 4000) { // Default duration 4 seconds
    if (!gameMessage) return;
    // Clear any existing timeout to prevent premature clearing
    if (gameMessageTimeout) { clearTimeout(gameMessageTimeout); gameMessageTimeout = null; }
    gameMessage.textContent = message;
    gameMessage.style.color = type === 'error' ? '#ff6666' : (type === 'success' ? '#90ee90' : '#ffdd88');
    // Set a timeout to clear the message after the duration
    // Don't auto-clear certain persistent messages
    const persistentMessages = ["Waiting for Opponent...", "Joining lobby...", "Fight!", "Time's up! Waiting...", "Waiting for another player..."];
    if (!persistentMessages.includes(message)) {
         gameMessageTimeout = setTimeout(() => {
             if (gameMessage.textContent === message) gameMessage.textContent = ''; // Clear only if unchanged
             gameMessageTimeout = null;
         }, durationMs);
    }
}
// --- End Updated displayGameMessage ---
...
function gameLoop() { try { if (!gameActive && !isCurrentlyWaiting) { stopGameLoop(); return; } clearCanvas(); drawBackground(); if(isCurrentlyWaiting) { drawWaitingText(); } else if (gameActive) { if (player1Castle?.ownerId) drawCastle(player1Castle.x, groundLevel, player1Castle.width, CASTLE_HEIGHT, PLAYER_1_COLOR); if (player2Castle?.ownerId) drawCastle(player2Castle.x, groundLevel, player2Castle.width, CASTLE_HEIGHT, PLAYER_2_COLOR); moveSoldiers(); checkCombat(); checkCastleHits(); activeSoldiers.forEach(drawSoldier); } animationFrameId = requestAnimationFrame(gameLoop); } catch (error) { console.error("CLIENT: Error in gameLoop:", error); gameActive = false; showGameOver("Runtime Error!"); }}
function startGameLoop() { try { if (!animationFrameId) { console.log("CLIENT: Starting animation loop..."); isCurrentlyWaiting = false; animationFrameId = requestAnimationFrame(gameLoop); } } catch (error) { console.error("CLIENT: Error starting game loop:", error); }}
...
// --- UI Listeners ---
function setupUIListeners() { try { joinButton?.addEventListener('click', () => { const nick = nicknameInput?.value.trim(); if (nick && ws && ws.readyState === WebSocket.OPEN) { if(connectionStatus) connectionStatus.textContent = 'Joining...'; ws.send(JSON.stringify({ type: 'join', nickname: nick })); if(nicknamePrompt) nicknamePrompt.style.display = 'none'; if(joinButton) joinButton.disabled = true; if(nicknameInput) nicknameInput.disabled = true; } else if (!nick) { if(connectionStatus) connectionStatus.textContent = 'Please enter a nickname.'; } else { if(connectionStatus) connectionStatus.textContent = 'Not connected yet.'; } }); submitWordButton?.addEventListener('click', submitWord); wordInput?.addEventListener('keypress', (event) => { if (event.key === 'Enter' && !submitWordButton?.disabled) submitWord(); }); wordInput?.addEventListener('input', () => { if(wordInput) wordInput.value = wordInput.value.toUpperCase(); }); playAgainButton?.addEventListener('click', () => { window.location.reload(); }); } catch (error) { console.error("CLIENT: Error setting up UI listeners:", error); } }

// --- WebSocket Setup & Connection ---
function connectWebSocket() { try { if (ws && (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING)) { return; } if (ws) { ws.close(); ws = null;} const proto=window.location.protocol; const wsProto=proto==="https:"?"wss:":"ws:"; let wsHost=window.location.hostname; let wsPort=window.location.port||(proto==="https:"?443:80); if((wsHost==='localhost'||wsHost==='127.0.0.1')&&wsPort!=8080){wsPort=8080;} const fullUrl=`${wsProto}//${wsHost}:${wsPort}`; console.log(`CLIENT: Attempting WS connection to: ${fullUrl}`); if(connectionStatus) connectionStatus.textContent=`Connecting...`; ws = new WebSocket(fullUrl); ws.onopen = () => { console.log('CLIENT: WS open.'); if(connectionStatus) connectionStatus.textContent = 'Connected! Enter nickname & Join.'; if(nicknamePrompt) nicknamePrompt.style.display = 'block'; if(joinButton) joinButton.disabled = false; if(nicknameInput) nicknameInput.disabled = false; }; ws.onmessage = (event) => { try { const msg = JSON.parse(event.data); handleServerMessage(msg); } catch (e) { console.error('CLIENT: Failed parse/handle msg:', event.data, e); } }; ws.onerror = (e) => { console.error('CLIENT: WS Error:', e); if(connectionStatus) connectionStatus.textContent = 'Connection Error.'; if (gameActive) showGameOver("Connection Error"); }; ws.onclose = (e) => { console.log(`CLIENT: WS Closed. Code:${e.code}`); isCurrentlyWaiting=false; gameActive=false; stopGameLoop(); clearInterval(currentTimerInterval); resetUI('Disconnected. Refresh?'); ws=null; playerId=null; gameId=null; playerNumber=null; }; } catch (e) { console.error("CLIENT: CRITICAL ERROR during connectWebSocket():", e); if(connectionStatus) connectionStatus.textContent = "WS Setup Error."; } }
...
function handleServerMessage(message) {
    try {
        switch (message.type) {
            ...
            case 'gameOver': isCurrentlyWaiting=false; if (!gameActive && gameOverScreen?.style.display !== 'none') break; console.log(`CLIENT: Received gameOver. Winner: ${message.winnerId}`); const win = message.winnerId === playerId; const winNN = win ? "You" : opponentNickname; const out = win ? `Victory!` : `${winNN} Wins!`; const rsn = message.reason === 'disconnect' ? ` (${opponentNickname} disconnected)` : ''; showGameOver(`${out}${rsn}`); break;
...
function init() { // Define init BEFORE DOMContentLoaded calls it
    try {
        console.log("CLIENT: init() started (V5 + Waiting Text)");
        // Assign DOM elements
