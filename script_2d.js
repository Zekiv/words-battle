// script_2d.js (Full Logic + Direct Connect + History Handling)
...
function updateWordHistoryUI(playedWords) {
    ...
    wordHistoryBox.scrollTop = wordHistoryBox.scrollHeight; // Scroll to bottom
}
...
function handleServerMessage(message) { // Define handleServerMessage BEFORE ws.onmessage uses it
    try {
        switch (message.type) {
            ...
            case 'initialLetters': case 'updateLetters': if(!gameActive) break; updateLettersUI(message.letters); break; // Simplified letter update
            case 'newRound': if (!gameActive) break; displayGameMessage('New Round!'); if(message.playedWords) updateWordHistoryUI(message.playedWords); if(message.timer) startRoundTimer(message.timer); if(wordFeedback) wordFeedback.textContent = ''; break; // Letters sent via updateLetters now
...
function init() { // Define init BEFORE DOMContentLoaded calls it
    try {
        console.log("CLIENT: init() started (Direct Connect V3 - Fixed Order + History).");
        // Assign DOM elements
        ...
        wordHistoryBox = document.getElementById('word-history-box'); wordHistoryList = document.getElementById('word-history-list'); // Assign history elements

        if (!canvas || !ctx || !connectionStatus || !nicknamePrompt || !joinButton || !submitWordButton || !wordHistoryBox || !wordHistoryList) {
             console.error("CLIENT: Critical UI elements missing during init!"); return;
         }
