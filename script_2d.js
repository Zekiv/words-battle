// script_2d.js (Complete - Draws Waiting Text on Canvas)
...
let isCurrentlyWaiting = false; // <<< Flag to track waiting state
...
// --- Function to draw centered waiting text on canvas ---
function drawWaitingText(message = "Waiting for Opponent...") {
    if (!ctx || !canvas) return;
    // Background is drawn by gameLoop before this is called
    ...
    const boxHeight = fontSize + boxPadding * 1.5; // Adjust as needed
    const boxX = (canvas.width - boxWidth) / 2;
    const boxY = (canvas.height - boxHeight) / 2 - 20; // Shift up slightly
    ...
    ctx.fillText(message, canvas.width / 2, boxY + boxHeight / 2);
}
...
// --- Animation Loop & Game Updates (Modified for Waiting State) ---
function gameLoop() {
    try {
        ...
        if (isCurrentlyWaiting) {
            // If waiting, draw the waiting text over the background
            drawWaitingText(); // Uses default text or last message from server
            // Keep requesting frames while waiting
            animationFrameId = requestAnimationFrame(gameLoop);
        } else if (gameActive) {
            // If game is active, draw all game elements
            ...
            moveSoldiers(); checkCombat(); checkCastleHits();
            activeSoldiers.forEach(drawSoldier);
            // Keep requesting frames while game is active
            animationFrameId = requestAnimationFrame(gameLoop);
        } else {
            // Neither waiting nor active game -> Stop the loop
             stopGameLoop();
             // Optional: Draw a final static background frame if needed? Usually handled by resetUI/showGameOver
             // drawBackground();
        }
    } catch (error) {
        console.error("CLIENT: Error in gameLoop:", error);
        gameActive = false; isCurrentlyWaiting = false; // Reset flags on error
        showGameOver("Runtime Error!");
        stopGameLoop();
    }
}
function startGameLoop() { try { if (!animationFrameId) { console.log("CLIENT: Starting animation loop (Active or Waiting)."); animationFrameId = requestAnimationFrame(gameLoop); } } catch (error) { console.error("CLIENT: Error starting game loop:", error); }}
    ...
    try {
        joinButton?.addEventListener('click', () => {
            ...
            if (nick && ws && ws.readyState === WebSocket.OPEN) {
                ...
                // Don't show waiting overlay div, just wait for server 'waiting' message
    ...
    try {
        switch (message.type) {
            ...
            case 'waiting':
                ...
                startGameLoop(); // Start loop to draw waiting text
                break;
            case 'gameStart':
                ...
                startGameLoop(); // <<< Start/ensure game loop is running for active game
                break;
...
function init() { // Define init BEFORE DOMContentLoaded calls it
    try {
        console.log("CLIENT: init() started (Waiting Text on Canvas V2).");
        // Assign DOM elements
        ...
        resetUI('Initializing...'); // Reset UI state
